...
void tdrDraw(TH1D* h, int marker=kFullCircle, int mcolor = kBlack,
             int lstyle=kSolid, int lcolor=-1,
             int fstyle=1001, int fcolor=kYellow+1){
  h->SetMarkerStyle(marker);
  h->SetMarkerColor(mcolor);
...
TString extraText3   = "Simulation"; // For Simulation Preliminary on two lines
...
TString lumi_13TeV_18 = "2018 59.83 fb^{-1}";
...
//      lumiText += lumi_8TeV;
    ...
    {
      lumiText += lumi_13TeV_BCD;
      lumiText += " (13 TeV)";
    }
  else if ( iPeriod==5 )
    {
      lumiText += lumi_13TeV_EF;
      lumiText += " (13 TeV)";
    }
  else if ( iPeriod==6 )
    {
      lumiText += lumi_13TeV_GH;
      lumiText += " (13 TeV)";
    }
  else if ( iPeriod==7 )
    {
      lumiText += lumi_13TeV_BCDEFGH;
      lumiText += " (13 TeV)";
    }
/*
  else if ( iPeriod==7 )
...
TCanvas* tdrmyDiCanvas(const char* canvName, TH1D *hup, TH1D *hdw, float iScale,
                   int iPeriod = 7, int iPos = 11) {
