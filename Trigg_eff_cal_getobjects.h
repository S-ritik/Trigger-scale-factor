#define Trigg_eff_cal_cxx
#include "Trigg_eff_cal.h"
...
float* Trigg_eff_cal::Electron_SF(float pt, float eta, string id){
...
float* Trigg_eff_cal::Get_PU_Weights(int npu){

	int bin_id = h_pileup_data->FindBin(npu);

	static float puweight[3] = {0,0,0};
	if(bin_id>=0 && bin_id<100){
		puweight[0] = h_pileup_data->GetBinContent(bin_id);
		puweight[1] = h_pileup_data_plus->GetBinContent(bin_id);
		puweight[2] = h_pileup_data_minus->GetBinContent(bin_id);
	}
	return puweight;
}

void Trigg_eff_cal::getmuons(std::vector<Muon> &vmuons, float ptcut=25, float etacut=2.5, int maxsize=njetmx, float dxy_cut=0.2, float dz_cut=0.5)
{

  for(int mu=0; mu<(nmuons); mu++){

    if(Muon_corrected_pt[mu]<ptcut) continue;
    ...
    vmuon.vetoid_tight = mu_id  && (muonminisoall[mu] < 0.1) /*&& fabs(muontrkvtx[mu])<dxy_cut && fabs(muondz[mu])<dz_cut*/;
    vmuon.pt = Muon_corrected_pt[mu];
...
void Trigg_eff_cal::getelectrons(std::vector<Electron> &velectrons, float ptcut=25, float etacut=2.5, int maxsize=njetmx, float dxy_cut=0.05, float dz_cut=0.1)
{
  ...
  for(int ie=0; ie<(nelecs); ie++) {
    ...
    velectron.p4.SetPtEtaPhiE(velectron.pt,velectron.eta,velectron.phi, ele[ie]);

/*    TString str = TString::Format(" el pt correction %f = %f charge %f & %f",Electron_eccalTrkEnergyPostCorr[ie] / ele[ie], Electron_eccalTrkEnergyPostCorr[ie] / velectron.p4.E(),elp[ie],velectron.p4.P()*elcharge[ie]);
    if(gProofServ) gProofServ->SendAsynMessage(str);*/


    velectron.p4 = velectron.p4 * (Electron_eccalTrkEnergyPostCorr[ie] / ele[ie]);
    velectron.pt = velectron.p4.Pt();
    velectron.eta = velectron.p4.Eta();
    velectron.phi = velectron.p4.Phi();
//    velectron.p = velectron.p4.P()*elcharge[ie];
...
void Trigg_eff_cal::getLeptons(std::vector<Lepton> &vleptons, std::vector<Muon> vmuons, std::vector<Electron> velectrons, float pt_cut=30)
{
...
void Trigg_eff_cal::convertintolep(Muon vmuon, Lepton &vlepton)
{
    vlepton.pt = vmuon.pt;
    vlepton.eta = vmuon.eta;
    vlepton.y = vmuon.y;
    vlepton.phi = vmuon.phi;
    vlepton.mass = vmuon.mass;
    vlepton.pdgId = (vmuon.p>0) ? -13 : 13;
    vlepton.p4 = vmuon.p4;
}

void Trigg_eff_cal::convertintolep(Electron velectron, Lepton &vlepton)
{
    vlepton.pt = velectron.pt;
    vlepton.eta = velectron.eta;
    vlepton.y = velectron.y;
    vlepton.phi = velectron.phi;
    vlepton.mass = velectron.mass;
    vlepton.pdgId = (velectron.p>0) ? -11 : 11;
    vlepton.p4 = velectron.p4;
}

void Trigg_eff_cal::getAK4jets(std::vector<AK4Jet> &Jets, float ptcut=30, float etacut=2.5, bool isMC=false, int maxsize=njetmx)
{
...
void Trigg_eff_cal::getAK8jets(std::vector<AK8Jet> &LJets, float ptcut=200, float etacut=2.5, bool isMC=false, int maxsize=njetmxAK8)
{
...
void Trigg_eff_cal::getAK4genjets(std::vector<AK4GenJet> &Jets, float ptcut=30, float etacut=2.5, int maxsize=njetmx)
{
  ...
  for(int ijet=0; ijet<(ngenjetAK4); ijet++){
    ...
    if(fabs(genjetAK4eta[ijet])>etacut /*|| (fabs(pfjetAK4eta[ijet])>1.4442 && fabs(pfjetAK4eta[ijet])<1.566)*/) continue;
    if(genjetAK4pt[ijet]<ptcut) continue;

    sJet.pt = genjetAK4pt[ijet];
    sJet.mass = genjetAK4mass[ijet];
    sJet.eta = genjetAK4eta[ijet];
    sJet.phi = genjetAK4phi[ijet];
    sJet.p4.SetPtEtaPhiM(sJet.pt,sJet.eta,sJet.phi,sJet.mass);
    sJet.y = sJet.p4.Rapidity();
    sJet.hadronFlavor = genjetAK4hadronflav[ijet];
    sJet.partonFlavor = genjetAK4partonflav[ijet];
...
/*void Trigg_eff_cal::LeptonJet_cleaning(std::vector <Electron> velectrons,std::vector <Muon> vmuons, float dR_cut=0.4)
 * {
...
void Trigg_eff_cal::getPartons(std::vector<GenParton> &GenPartons, int maxsize=npartmx)
{
...
void Trigg_eff_cal::getLHEParticles(std::vector<LHEparticle> &lheparticles, int maxsize=npartmx)
{
...
void Trigg_eff_cal::getLHETops(std::vector<GenParton> &LHETops, std::vector<GenParton> GenPartons)
{
...
void Trigg_eff_cal::getGENTops(vector<TopQuark> &gentops, vector<GenParton> genpartons)  // with daughters after shower
{
...
void Trigg_eff_cal::TopAssignment_toJet(std::vector<AK8Jet> &LJets, std::vector<GenParton> lhetops, std::vector<TopQuark> gentops)
{
  for(unsigned ijet=0; ijet<LJets.size(); ijet++){
    ...
    for(unsigned itop=0; itop<gentops.size(); itop++){
      ...
      if(abs(gentops[itop].daughter[0].pdgId)==11 || abs(gentops[itop].daughter[0].pdgId)==11 || abs(gentops[itop].daughter[0].pdgId)==11){
...
void Trigg_eff_cal::AssignGen(std::vector<AK8Jet> &LJets, std::vector<GenParton> GenPartons){
...
bool Trigg_eff_cal::isBJet(AK4Jet jet, float btag_cut)
{
...
void Trigg_eff_cal::ReadTagger(std::vector<AK8Jet> &LJets, std::vector<Lepton> vleptons, std::vector<Muon> vmuons, std::vector<Electron> velectrons, TMVA::Reader *reader_electron, TMVA::Reader *reader_muon){
...
void Trigg_eff_cal::Match_trigger(vector<Single_Trigger> vsinglelep_trig,vector<Double_Trigger> vdoublelep_trig, vector<std::pair<int,TLorentzVector>> TrigRefObj,Lepton lepcand_1, Lepton lepcand_2, vector<AK4Jet> Jets, bool &trig_threshold_pass, bool &trig_matching_pass, vector<TH1D*> &hist_init,vector<TH2D*> &hist2d)
{
  ...
  if(vdoublelep_trig.size()<1 && vsinglelep_trig.size()<1) {
	...
	if (vdoublelep_trig[ihlt].double_hlts && (( vdoublelep_trig[ihlt].double_pids[0]==abs(lepcand_1.pdgId) &&  vdoublelep_trig[ihlt].double_pids[1]==abs(lepcand_2.pdgId)) || ((vdoublelep_trig[ihlt].double_pids[1]==abs(lepcand_1.pdgId) && vdoublelep_trig[ihlt].double_pids[0]==abs(lepcand_1.pdgId))))) {
